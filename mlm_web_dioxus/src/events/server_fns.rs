...
pub async fn get_events_data(
    ...
    let page_size_val = page_size.unwrap_or(MAX_PAGE_SIZE).clamp(1, MAX_PAGE_SIZE);
    ...
    if no_filters {
        ...
        for event_res in events {
            ...
            let db_torrent: Option<mlm_core::Torrent> = if let Some(id) = &db_event.torrent_id {
                r.get().primary(id.clone()).server_err()?
            ...
            if let Some(ref t) = db_torrent {
                db_replacement = t
                    .replaced_with
                    .clone()
                    .map(|(id, _)| r.get().primary(id).server_err())
                    .transpose()?
                    .flatten();
            }
    ...
    for event_res in events {
        ...
        let page_end = from_val.saturating_add(page_size_val);
        let in_page = total_matching >= from_val && total_matching < page_end;
