...
use mlm_core::autograbber::{
    PreparedTorrentMetaUpdate, finalize_torrent_meta_update, queue_torrent_meta_update,
};
#[cfg(feature = "server")]
...
pub async fn apply_tag_filter_action(
    ...
    let total_batches = total_torrents.div_ceil(BATCH_SIZE.max(1));
    let read_tx = context
        .db()
        .r_transaction()
        .map_err(|e| ServerFnError::new(e.to_string()))?;
    ...
    for torrent in torrent_iter {
        ...
        seen_count += batch.len();
        process_tag_filter_batch(
            &context,
            &config,
            &qbit,
            qbit_conf,
...
async fn process_tag_filter_batch(
    context: &mlm_core::Context,
    config: &mlm_core::config::Config,
    qbit: &qbit::Api,
    qbit_conf: &mlm_core::config::QbitConfig,
    tag_filter: &mlm_core::config::TagFilter,
    mam_semaphore: &Arc<Semaphore>,
    batch: &[mlm_core::Torrent],
    ...
    tracing::info!(
        ...
        batch_idx + 1,
        total_batches,
        seen_count.saturating_sub(batch.len()) + 1,
        seen_count
    );

    let mut matched_torrents = Vec::with_capacity(batch.len());
    ...
    for torrent in batch {
        match tag_filter.filter.matches_lib(torrent) {
            Ok(matches) => {
                if matches {
                    matched_torrents.push(torrent.clone());
                }
            }
            Err(_) => {
    ...
    if !meta_updates.is_empty() {
        ...
        for (torrent, mam_torrent, new_meta) in meta_updates {
            match queue_torrent_meta_update(
                &rw,
                Some(&mam_torrent),
                torrent,
                new_meta,
            ...
            .map_err(|e| ServerFnError::new(e.to_string()))?
            {
                ...
                PreparedTorrentMetaUpdate::Pending(pending) => {
                    wrote_changes = true;
    ...
    if !matched_torrents.is_empty() {
        ...
        let hashes: Vec<&str> = matched_torrents.iter().map(|t| t.id.as_str()).collect();
