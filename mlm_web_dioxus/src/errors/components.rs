...
use crate::components::{
    ...
    set_location_query_string, update_row_selection,
};
...
pub fn ErrorsPage() -> Element {
    ...
    let mut last_errors_trigger = use_signal(|| 0u32);
    ...
    use_effect(move || {
        let current_trigger = *ERRORS_UPDATE_TRIGGER.read();
        if *last_errors_trigger.read() != current_trigger {
            last_errors_trigger.set(current_trigger);
            errors_data.restart();
        }
    });
