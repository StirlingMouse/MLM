...
pub fn DownloadButtons(props: DownloadButtonsProps) -> Element {
    ...
    let handle_download = Callback::new(move |(wedge, success_msg): (bool, String)| {
        loading.set(true);
        on_status.call((String::new(), false));
        spawn(async move {
            match select_torrent_action(mam_id, wedge).await {
                Ok(_) => {
                    on_status.call((success_msg, false));
                    on_refresh.call(());
                }
                Err(e) => {
                    on_status.call((format!("Selection failed: {e}"), true));
                }
            }
            loading.set(false);
        });
    ...
    rsx! {
        if let Some(label) = freeleech_label {
            button {
                ...
                onclick: move |_| {
                    handle_download.call((false, "Torrent queued for download".to_string()));
                },
                if *loading.read() {
            ...
            if props.mode == DownloadButtonMode::Compact {
                ...
                if props.can_wedge {
                    button {
                        ...
                        disabled: is_disabled,
                        onclick: move |_| {
                            handle_download.call((true, "Torrent queued with wedge".to_string()));
                        },
