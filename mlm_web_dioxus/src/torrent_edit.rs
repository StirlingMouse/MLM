...
pub async fn get_torrent_meta_edit_data(id: String) -> Result<TorrentMetaEditForm, ServerFnError> {
    ...
    Ok(TorrentMetaEditForm {
        ...
        uploaded_at_unix: meta
            .uploaded_at
            .map(|uploaded_at| uploaded_at.0.unix_timestamp().to_string())
            .unwrap_or_default(),
    })
}
...
pub async fn update_torrent_meta_edit_data(form: TorrentMetaEditForm) -> Result<(), ServerFnError> {
    ...
    let uploaded_at = if form.uploaded_at_unix.trim().is_empty() {
        None
    } else {
        let uploaded_at_unix =
            form.uploaded_at_unix.trim().parse::<i64>().map_err(|e| {
                ServerFnError::new(format!("invalid uploaded_at unix timestamp: {e}"))
            })?;
        Some(
            time::UtcDateTime::from_unix_timestamp(uploaded_at_unix).map_err(|e| {
                ServerFnError::new(format!("invalid uploaded_at unix timestamp: {e}"))
            })?,
        )
    };
    ...
    let meta =
        mlm_db::TorrentMeta {
            ...
            source,
            uploaded_at: uploaded_at.map(Into::into),
        };
