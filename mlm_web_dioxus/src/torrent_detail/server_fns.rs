...
async fn read_library_files(
    ...
    while let Some(entry) = entries.next_entry().await.server_err()? {
...
pub async fn get_qbit_data(id: String) -> Result<Option<super::types::QbitData>, ServerFnError> {
    ...
    Ok(Some(super::types::QbitData {
        ...
        is_paused: matches!(
            qbit_torrent.state,
            TorrentState::StoppedDownloading | TorrentState::StoppedUploading
        ),
...
pub async fn remove_seeding_files_action(id: String) -> Result<(), ServerFnError> {
    ...
    let library_files_set: std::collections::HashSet<_> =
        read_library_files(torrent.library_path.as_deref())
            .await?
            .into_iter()
            .collect();
    ...
    if !files_to_remove.is_empty() {
        for file_id in files_to_remove {
            ...
            if fs::try_exists(&path).await.server_err()? {
                fs::remove_file(path).await.server_err()?;
            }
        }
    }
