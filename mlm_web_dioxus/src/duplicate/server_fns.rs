...
pub async fn get_duplicate_data(
    ...
    let mut duplicates = r
        ...
        .filter_map(|result| match result {
            ...
            Err(err) => {
                tracing::warn!("Skipping duplicate row after scan error: {err}");
                None
            }
        })
        .filter(|t| {
...
pub async fn apply_duplicate_action(
    ...
    match action {
        DuplicateBulkAction::Replace => {
            ...
            let r = context.db().r_transaction().server_err()?;
            let replace_rows = torrent_ids
                .into_iter()
                .map(|mam_id| {
                    let duplicate_torrent = r
                        .get()
                        .primary::<DuplicateTorrent>(mam_id)
                        .server_err()?
                        .ok_or_else(|| ServerFnError::new("Duplicate torrent not found"))?;
                    let hash = duplicate_torrent
                        .duplicate_of
                        .clone()
                        .ok_or_else(|| ServerFnError::new("No duplicate_of set"))?;
                    let duplicate_of = r
                        .get()
                        .primary::<Torrent>(hash)
                        .server_err()?
                        .ok_or_else(|| ServerFnError::new("Could not find original torrent"))?;
                    Ok((duplicate_torrent, duplicate_of))
                })
                .collect::<Result<Vec<_>, ServerFnError>>()?;
            drop(r);

            let mut inserts = Vec::with_capacity(replace_rows.len());
            for (duplicate_torrent, _) in &replace_rows {
            ...
            for insert in inserts {
                rw.insert(insert).server_err()?;
            }
            for (duplicate_torrent, _) in &replace_rows {
                rw.remove(duplicate_torrent.clone()).server_err()?;
            }
            rw.commit().server_err()?;

            for (_, duplicate_of) in replace_rows {
